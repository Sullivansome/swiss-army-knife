...
export function UnitConverterTool({ labels }: Props) {
  ...
  const [lengthUnit, setLengthUnit] = useState<(typeof lengthUnits)[number]["key"]>("cm");
  const [weightUnit, setWeightUnit] = useState<(typeof weightUnits)[number]["key"]>("kg");

  const lengthResults = useMemo(() => {
    const base = value * lengthUnits.find((unit) => unit.key === lengthUnit)!.factor;
  ...
  const weightResults = useMemo(() => {
    const base = value * weightUnits.find((unit) => unit.key === weightUnit)!.factor;
    ...
    <div className="space-y-4">
      ...
      {category === "length" ? (
        <ConverterSection
          ...
          units={lengthUnits.map((unit) => ({ key: unit.key, label: labels.lengthUnits[unit.key] }))}
          results={lengthResults.map((item) => ({ label: labels.lengthUnits[item.key], value: item.value }))}
          labels={{ value: labels.value, unit: labels.unit, results: labels.results }}
        />
      ) : null}
      ...
      {category === "weight" ? (
        <ConverterSection
          ...
          units={weightUnits.map((unit) => ({ key: unit.key, label: labels.weightUnits[unit.key] }))}
          results={weightResults.map((item) => ({ label: labels.weightUnits[item.key], value: item.value }))}
          labels={{ value: labels.value, unit: labels.unit, results: labels.results }}
        />
      ) : null}
      ...
      {category === "shoe" ? (
        <div className="space-y-2">
          <p className="text-sm font-semibold text-foreground">{labels.shoe.title}</p>
      ...
      {category === "clothing" ? (
        <div className="space-y-2">
          <p className="text-sm font-semibold text-foreground">{labels.clothing.title}</p>
          <div className="overflow-auto rounded-xl border">
            <table className="min-w-full text-sm">
              <thead>
                <tr className="bg-muted/40 text-left">
                  <th className="px-4 py-2 font-medium">{labels.clothing.international}</th>
                  <th className="px-4 py-2 font-medium">{labels.clothing.us}</th>
                  <th className="px-4 py-2 font-medium">{labels.clothing.eu}</th>
                  <th className="px-4 py-2 font-medium">{labels.clothing.cn}</th>
                </tr>
              </thead>
              <tbody>
...
function ConverterSection({ value, unit, onValueChange, onUnitChange, units, results, labels }: ConverterSectionProps) {
  return (
    <div className="space-y-3">
      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1">
          <label className="text-sm font-medium text-foreground">{labels.value}</label>
          <input
            type="number"
            value={value}
        ...
        <div className="space-y-1">
          <label className="text-sm font-medium text-foreground">{labels.unit}</label>
          <select
            value={unit}
      ...
      <div className="space-y-2">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {results.map((row) => (
            <div key={row.label} className="rounded-xl border bg-card p-3 text-center">
              <p className="text-sm text-muted-foreground">{row.label}</p>
              <p className="mt-2 text-xl font-semibold text-foreground">{row.value}</p>
            </div>
          ))}
        </div>
      </div>
